...
namespace Seb.Fluid2D.Simulation
{
    public class FluidSim2D : MonoBehaviour
    {
        ...
        void Init()
        {
            ...
            positionsCPU = new float2[numParticles];
        ...
        public int CountParticlesInsideRect(Rect rect)
        {
            // Read the GPU position buffer into the CPU array
            positionBuffer.GetData(positionsCPU);

            int count = 0;
            for (int i = 0; i < numParticles; i++)
            {
                float2 pos = positionsCPU[i];
